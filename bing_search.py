...
# - Requires a Bing Search API key via Windows Azure Marketplace
...
BING_KEY = tpo.getenv_text("BING_KEY", "",
                           "API key (via Microsoft Azure)")
...
def bing_search(query, key=None, use_json=False, search_type=None, topn=None, non_phrasal=False):
    ...
    ## TODO: see of old search types RelatedSearch and Composite still supported
    ...
    ## OLD: credentials = (":%s" % key).encode("base64")[:-1]
    ## TEST:
    ## try:
    ##     colon_key_bytes = (":" + key).encode("utf-8")
    ##     encoded_colon_key = base64.b64encode(colon_key_bytes)
    ##     ## OLD: debug.assertion(encoded_key[-1] == "\n")
    ##     ## OLD: encoded_key = encoded_key[:-1]
    ...
    ## OLD: format_spec = "&textFormat=json" if use_json else ""
...
def main():
    ...
    while (i < len(sys.argv) and (sys.argv[i][0] == "-")):
        ...
        i += 1
    if (show_usage):
        print("Usage: %s [--json] [--image] [--type label] query_word ..." % sys.argv[0])
        print("Notes:")
